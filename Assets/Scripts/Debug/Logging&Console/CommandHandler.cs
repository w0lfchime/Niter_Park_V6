...
public static class CommandHandler
{
    ...
    public static void RegisterCommand(string command, Action<string[]> callback)
    {
        if (!commands.ContainsKey(command))
        {
            ...
            LogCore.Log($"Command '{command}' registered.");
        }
        else
        {
            LogCore.Log($"Command '{command}' is already registered.");
        }
    }
    ...
    public static void ExecuteCommand(string commandInput)
    {
        ...
        {
            LogCore.Log("Command cannot be empty.");
            return;
        }
        ...
        {
            LogCore.Log("Invalid command. All commands must start with a '/'.");
            return;
        }
        ...
        {
            LogCore.Log($"Command '{command}' not recognized.");
        }
    }
